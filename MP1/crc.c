...
struct Reply process_command(const int sockfd, char* command, const int port);
...
int main(int argc, char** argv) 
{
	...
	while (1) {
		...
		struct Reply reply = process_command(sockfd, command, atoi(argv[2]));
...
int connect_to(const char *host, const int port)
{
	...
	if (status == -1){
		printf("Error: Unable to connect to this host.\n");
	}
...
struct Reply process_command(const int sockfd, char* command, const int port)
{
	...
	// ------------------------------------------------------------
	// printf("Command u inputted was: %s\n", command);
	send(sockfd, command, sizeof(command)+100, 0);

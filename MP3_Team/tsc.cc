...
class Client : public IClient
{
    ...
    private:
        ...
        std::unique_ptr<SNSCoord::Stub> coord_stub_;
        std::unique_ptr<SNSService::Stub> service_stub_;
...
int Client::connectTo()
{
	...
	//Stub for communicating with coordinator
    std::string login_info = hostname + ":" + port;
    coord_stub_ = std::unique_ptr<SNSCoord::Stub>(SNSCoord::NewStub(
               grpc::CreateChannel(
                    login_info, grpc::InsecureChannelCredentials())));
    
    login_info = Access(); //return the correct IP and Port to use in the later functions
    //create new stub with that IP and Port
    std::cout << "At line 123" << std::endl;
    service_stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
               grpc::CreateChannel(
                    login_info, grpc::InsecureChannelCredentials())));
    std::cout << "At line 127" << std::endl;
    IReply ire = Login();
...
std::string Client::Access() {
    CoordRequest request;
    request.set_client_id(username);
    
    CoordReply reply;
    ClientContext context;
    Status status = coord_stub_->Access(&context, request, &reply);
    IReply ire;
    ire.grpc_status = status;
    ...
    if(status.ok()){
        ...
        server_ip = reply.server_ip_address();
        server_port = reply.server_port();
    }
    std::string return_statement = server_ip + ":" + server_port;
    std::cout << "IP and Port returned: " << return_statement << std::endl;
...
IReply Client::List() {
    ...
    Status status = service_stub_->List(&context, request, &list_reply);
    IReply ire;
    ire.grpc_status = status;
...
IReply Client::Follow(const std::string& username2) {
    ...
    Status status = service_stub_->Follow(&context, request, &reply);
...
IReply Client::UnFollow(const std::string& username2) {
    ...
    Status status = service_stub_->UnFollow(&context, request, &reply);
    IReply ire;
    ire.grpc_status = status;
...
IReply Client::Login() {
    ...
    Status status = service_stub_->Login(&context, request, &reply);
...
void Client::Timeline(const std::string& username) {
    ...
    std::shared_ptr<ClientReaderWriter<Message, Message>> stream(
            service_stub_->Timeline(&context));
